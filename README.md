...
Для автоматизации создания компонентов используется gulp-таск `create`. Для его запуска необходимо в консоли ввести команду `COMPONENT=<new_component_name> gulp create` и нажать Enter. После этого в папке `src/components` создается папка с названием компонента и в ней создаются файлы pug, scss. Название имен файлов совпадает с названием компонента. Все это сделано для легкого переиспользования. Например, компоненты при таком подходе легко копируются из проекта в проект.
